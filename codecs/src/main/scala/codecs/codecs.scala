...
enum Json:
  ...
  /**
   * Decodes this JSON value into a value of type `A` by using
...
trait DecoderInstances:
  ...
  given Decoder[Unit] =
    Decoder.fromFunction {x => Option[Unit](())}

  /** A decoder for `Int` values. */

  given Decoder[Int] =
    Decoder.fromFunction{
      case Json.Num(x) => if x.isValidInt then Option[Int](x.toInt) else Option.empty[Int]
  ...
  given Decoder[String] =
    Decoder.fromFunction{
      case Json.Str(x) => Option[String](x)
  ...
  given Decoder[Boolean] =
    Decoder.fromFunction{
      case Json.Bool(x) => Option[Boolean](x)
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(x) => 
        val OptionsList = x.map(a => decoder.decode(a))
        if OptionsList.forall(!_.isEmpty) then
          Option[List[A]](OptionsList.map(_.get))
        else
          Option.empty[List[A]]
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.fromFunction{
      case Json.Obj(x) =>
        val pName = summon[Decoder[String]].decode(x("name"))
        val pAge = summon[Decoder[Int]].decode(x("age"))
        Option[Person](Person(pName.get, pAge.get))
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] = 
    Decoder.fromFunction{
      case Json.Obj(x) => 
        val pList = summon[Decoder[List[Person]]].decode(x("people"))
        Option[Contacts](Contacts(pList.get))
        
      case _ => Option.empty[Contacts]
    }
