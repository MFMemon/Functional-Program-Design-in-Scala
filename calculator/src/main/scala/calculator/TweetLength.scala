...
object TweetLength extends TweetLengthInterface:
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] =
    ...
    val tweetSig = Signal.Var(MaxTweetLength)
    Signal{
      tweetSig() = MaxTweetLength - tweetLength(tweetText.currentValue)
      tweetText()
    }
    tweetSig
  ...
  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] =
    ...
    val colorSig = Signal.Var[String](null)

    Signal{
      val sigValCV = remainingCharsCount.currentValue
      if sigValCV >= 15 then
        colorSig() = "green"
      else if 0 <= sigValCV && sigValCV <= 14 then
        colorSig() = "orange"
      else
        colorSig() = "red"
      remainingCharsCount()
