...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = 
    val legal_neighbors = b.legalNeighbors.to(LazyList)
    legal_neighbors.map(x => (x._1, x._2 :: history))

  /**
   * This function returns the list of neighbors without the block
   * positions that have already been explored. Used to
   ...
   *
   * The resulting lazy list would be sorted by ascending path length,
   ...
   * constructs the correctly sorted lazy list.
   */
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val neighbors = 
      for 
        p <- initial
        nh = neighborsWithHistory(p._1, p._2)
        nn <- newNeighborsOnly(nh, explored)
      yield
        nn
   ...
   * position.
   */
  lazy val solution: List[Move] = if pathsToGoal.isEmpty then List[Move]() else pathsToGoal.head._2.reverse
